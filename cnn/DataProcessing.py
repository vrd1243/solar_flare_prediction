...
class sunspotImageDataSetBrStacked(torch.utils.data.Dataset):

    def __init__(self, df, root_dir, transform):

        self.name_frame = df.iloc[:, 0:2]
    ...
    def __getitem__(self, idx):
        ...
        image_stack = np.zeros((2,256,256))
        for i in range(2):
            img_name = os.path.join(self.root_dir, self.name_frame.iloc[idx][i])
            image = Image.open(img_name)
            image = image.resize((256, 256), Image.AFFINE)
            image = self.transform(image)
            image_stack[i, :,:] = np.array(image)[1,:,:].reshape((1, image.shape[1], image.shape[2]))
...
def generateTrainValidData(df, root_dir, splitType = 'by_harpnum'):
    ...
    dataSetFn = sunspotImageDataSetBrStacked
